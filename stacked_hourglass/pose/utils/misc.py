...
def to_torch(ndarray):
    """
    Convert a numpy ndarry to a pytorch tensor.
    """
...
def save_checkpoint(state, preds, is_best, checkpoint='checkpoint', filename='checkpoint.pth.tar', snapshot=None):
    """
    Save a dictionary of the current state of training 'state'.
    """
...
def save_pred(preds, checkpoint='checkpoint', filename='preds_valid.mat'):
    """
    Save a set of predictions made during training. (Used in the eval code to evaluate the network w.r.t. PCKh metric).
    """
    preds = to_numpy(preds)
    filepath = os.path.join(checkpoint, filename)
